...
**Invoice numbers** are generated server-side in `app/actions/invoices.ts` as `INV-YYYYMMDD-NN`, always computed in the `Asia/Colombo` timezone (not server-local time), with a per-day counter and retry-on-unique-conflict loop (handles two invoices created in the same second).
...
- `public/pre-printed-invoice-form.webp` — an actual photo of the real stationery. It's shown as an on-screen-only background image for visual accuracy; it is **never** part of what actually prints (hidden via `print:hidden` CSS), since the real paper already has it.
